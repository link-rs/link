...
// STM32F405-specific I2SEXT register addresses (from STM32F405RG data sheet)
#[cfg(stm32f405)]
...
// STM32F405-specific DMA channel mapping for I2SEXT (from STM32F405RG data sheet)
#[cfg(stm32f405)]
...
// STM32F405-specific SdExt pin assignment (from STM32F405RG data sheet, p. 64)
#[cfg(stm32f405)]
...
pub enum Error {
    ...
    /// Overrun
    Overrun,
    /// The ring buffers have become unsync'ed from DMA.
    DmaUnsynced,
}
    ...
    pub async fn read_write(&mut self, tx_data: &[W], rx_data: &mut [W]) -> Result<(), Error> {
        match (&mut self.tx_ring_buffer, &mut self.rx_ring_buffer) {
            (Some(tx_ring), Some(rx_ring)) => {
                ...
                match tx_r.and(rx_r) {
                    Ok(_) => Ok(()),
                    Err(ringbuffer::Error::Overrun) => Err(Error::Overrun),
                    Err(ringbuffer::Error::DmaUnsynced) => Err(Error::DmaUnsynced),
                }
            }
